...
export function PaymentsView() {
  ...
  const run = useMutation({
    mutationFn: () => api.runPayments(period),
    onSuccess: async (result) => {
      setNotice(`Đã tạo ${result.created} khoản, bỏ qua ${result.skipped} khoản đã chốt. Tổng mới: ${formatMoney(result.total_amount)}.`);
  ...
  const createPrice = useMutation({
    mutationFn: () => api.createOilPrice({ unit_price: Number(unitPrice), note: note || undefined }),
    onSuccess: async () => {
      setUnitPrice('');
      setNote('');
      setNotice('Đã áp dụng đơn giá mới từ thời điểm hiện tại.');
      await queryClient.invalidateQueries({ queryKey: ['oil-prices'] });
    },
  });
  ...
  function submitPrice(event: FormEvent) {
    ...
    if (Number(unitPrice) <= 0) {
      setNotice('Vui lòng nhập đơn giá lớn hơn 0.');
      return;
    }
    if (window.confirm(`Áp dụng đơn giá ${formatMoney(Number(unitPrice))}/lít từ bây giờ?`)) createPrice.mutate();
  }
  ...
  return <AdminShell>
    <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div><p className="text-sm font-semibold text-emerald-700">Đối soát tiền quán</p><h2 className="mt-1 text-3xl font-bold">Thanh toán theo tuần</h2></div>
      <div className="flex flex-wrap gap-2"><input aria-label="Kỳ thanh toán" type="week" value={period} onChange={(event) => setPeriod(event.target.value)} className="min-h-11 rounded-xl border border-slate-300 bg-white px-3" /><button type="button" disabled={run.isPending || !period} onClick={() => run.mutate()} className="min-h-11 rounded-xl bg-emerald-700 px-5 font-semibold text-white disabled:opacity-50">{run.isPending ? 'Đang chốt…' : 'Chốt kỳ'}</button></div>
    </div>
    ...
    <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="text-lg font-bold">Chi tiết theo quán</h3>{!payments.data?.data.length ? <div className="mt-4"><EmptyState message="Kỳ này chưa được chốt hoặc chưa có giao dịch đạt chất lượng." /></div> : <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="border-b text-xs uppercase text-slate-400"><tr><th className="pb-3">Quán</th><th className="pb-3">Ngày thu</th><th className="pb-3">Số lít</th><th className="pb-3">Đơn giá đã chốt</th><th className="pb-3">Thành tiền</th><th className="pb-3">Trạng thái</th><th className="pb-3">Thao tác</th></tr></thead><tbody>{payments.data.data.map((payment) => <tr key={payment.id} className="border-b last:border-0"><td className="py-4 font-semibold">{payment.merchant_name}</td><td className="py-4">{formatDate(payment.collected_at)}</td><td className="py-4">{formatLiters(payment.liters)}</td><td className="py-4">{formatMoney(payment.unit_price)}/lít</td><td className="py-4 font-bold">{formatMoney(payment.amount)}</td><td className="py-4"><Badge tone={payment.status === 'PAID' ? 'green' : payment.status === 'CANCELLED' ? 'red' : 'orange'}>{statusLabel[payment.status]}</Badge></td><td className="py-4">{payment.status === 'PENDING' ? <button type="button" disabled={markPaid.isPending} onClick={() => markPaid.mutate(payment.id)} className="min-h-11 rounded-xl border border-emerald-600 px-3 font-semibold text-emerald-700 disabled:opacity-50">Đánh dấu đã trả</button> : '—'}</td></tr>)}</tbody></table></div>}</section>
    <section className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5"><h3 className="text-lg font-bold">Đơn giá dầu</h3><p className="mt-2 text-sm text-amber-900">Đổi đơn giá chỉ áp dụng cho giao dịch phát sinh từ thời điểm mới. Các khoản đã chốt giữ nguyên đơn giá và thành tiền.</p><p className="mt-3">Hiện hành: <strong>{currentPrice ? `${formatMoney(currentPrice.unit_price)}/lít` : 'Chưa cấu hình'}</strong></p><form onSubmit={submitPrice} className="mt-4 grid gap-3 md:grid-cols-[180px_1fr_auto]"><input aria-label="Đơn giá mới" type="number" min="1" step="1" value={unitPrice} onChange={(event) => setUnitPrice(event.target.value)} placeholder="Đơn giá mới" className="min-h-11 rounded-xl border border-slate-300 bg-white px-3" /><input aria-label="Ghi chú đơn giá" value={note} onChange={(event) => setNote(event.target.value)} placeholder="Ghi chú (không bắt buộc)" className="min-h-11 rounded-xl border border-slate-300 bg-white px-3" /><button disabled={createPrice.isPending} className="min-h-11 rounded-xl bg-ink px-5 font-semibold text-white disabled:opacity-50">{createPrice.isPending ? 'Đang lưu…' : 'Đổi đơn giá'}</button></form><div className="mt-4 space-y-2">{prices.data?.map((price) => <div key={price.id} className="flex flex-wrap justify-between gap-2 border-t border-amber-200 pt-2 text-sm"><span>{formatMoney(price.unit_price)}/lít · từ {formatDate(price.effective_from)}</span><span>{price.effective_to ? `đến ${formatDate(price.effective_to)}` : 'Đang áp dụng'}</span></div>)}</div></section>
  </AdminShell>;
}
