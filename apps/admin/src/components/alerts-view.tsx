  ...
  return <AdminShell>
    ...
    <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      {!alerts.data?.data.length ? <EmptyState message="Không có cảnh báo phù hợp." /> : <div className="space-y-3">
        {alerts.data.data.map((alert) => <article key={alert.id} className="flex flex-col gap-3 rounded-xl border border-slate-200 p-4 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <Badge tone={alert.type === 'COLLECTION_LITERS_DEVIATION' || alert.type === 'DELIVERY_VARIANCE' ? 'orange' : 'red'}>{alertLabel(alert.type)}</Badge>
              {alert.severity && <Badge tone={alert.severity === 'HIGH' ? 'red' : 'orange'}>{alert.severity}</Badge>}
          ...
          {!alert.resolved_at && <button className="min-h-11 rounded-xl bg-ink px-4 text-sm font-semibold text-white disabled:opacity-50" disabled={resolve.isPending} onClick={() => resolve.mutate(alert.id)}>Đánh dấu đã xử lý</button>}
        </article>)}
      </div>}
    </section>
  </AdminShell>;
