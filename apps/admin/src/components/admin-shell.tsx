...
const links = [
  ...
  ['/ai-performance', 'Hiệu quả AI'],
  ['/alerts', 'Cảnh báo'],
  ['/stations', 'Trạm'],
  ['/wards', 'Phường / Địa bàn'],
  ['/merchants', 'Quán'],
  ['/containers', 'Quản lý can'],
  ['/approvals', 'Duyệt quán'],
    ...
    <div className="min-h-screen bg-sand text-ink md:flex">
        ...
        <nav className="grid grid-cols-2 gap-2 md:block md:space-y-1">
          {links.map(([href, label]) => (
            <Link
              key={href}
              href={href}
              className={`flex items-center justify-between rounded-xl px-3 py-2.5 text-sm transition ${pathname === href ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-white/10 hover:text-white'}`}
            >
              <span>{label}</span>
              {href === '/approvals' && pending.data && pending.data.meta.total > 0 ? (
                <span className="rounded-full bg-amber-300 px-2 py-0.5 text-xs font-bold text-ink">
      ...
      <div className="min-w-0 flex-1">
        <header className="flex items-center justify-between border-b border-slate-200 bg-white px-5 py-4 md:px-8">
          <div>
            <p className="text-xs uppercase tracking-widest text-slate-400">Bảng điều hành</p>
            <p className="font-semibold">Xin chào, {user.name ?? 'Quản trị viên'}</p>
          </div>
          <button
