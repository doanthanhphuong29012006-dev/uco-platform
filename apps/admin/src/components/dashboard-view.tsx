    ...
    <AdminShell>
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-bold">Lít thu gom 14 ngày gần nhất</h3>
            <p className="text-sm text-slate-500">Ngày không có giao dịch vẫn hiển thị 0.</p>
          </div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.25fr_1fr]">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h3 className="text-lg font-bold">10 giao dịch gần nhất</h3>
              ...
              <table className="w-full min-w-[980px] text-left text-sm">
                ...
                <tbody>
                  {overview.data.recent_transactions.map((transaction) => (
                    <tr
                      key={transaction.id}
                      className={`border-b last:border-0 ${transaction.suspected_adulteration ? 'bg-red-50' : ''}`}
                    >
                      <td className="py-3 font-semibold">{transaction.merchant_name}</td>
                      <td className="py-3">{transaction.collector_name ?? '—'}</td>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h3 className="text-lg font-bold">Quán trong khu vực</h3>
