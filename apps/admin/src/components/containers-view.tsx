...
function friendlyCreateError(error: unknown): string {
  if (!(error instanceof ApiError)) return 'Không thể tạo can. Vui lòng thử lại.';
  if (error.code === 'WARD_NOT_FOUND') return 'Phường đã chọn không còn tồn tại. Vui lòng chọn lại phường.';
  if (error.code === 'QR_CODE_ALREADY_EXISTS') return 'Mã QR đã tồn tại. Vui lòng thử tạo lại.';
  return error.message || 'Không thể tạo can. Vui lòng thử lại.';
}

function friendlyReturnError(error: unknown): string {
  if (!(error instanceof ApiError)) return 'Không thể trả can về quán. Vui lòng thử lại.';
  if (error.code === 'CONTAINER_NOT_AT_STATION') return 'Can không còn ở tại trạm. Vui lòng tải lại danh sách.';
  if (error.code === 'MERCHANT_REQUIRED') return 'Vui lòng chọn quán nhận can.';
  if (error.code === 'MERCHANT_NOT_APPROVED') return 'Quán nhận can chưa được duyệt hoặc không còn hoạt động.';
  return error.message || 'Không thể trả can về quán. Vui lòng thử lại.';
}
...
export function ContainersView() {
  ...
  const returnMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setReturnNote('');
      setReturnMerchantId('');
      void queryClient.invalidateQueries({ queryKey: ['admin-containers'] });
    },
  });
  const selected = containers.data?.data.find((item) => item.id === selectedId) ?? null;

  useEffect(() => {
    if (wards.data?.length === 1 && !wardId) setWardId(wards.data[0].id);
  }, [wardId, wards.data]);
  ...
  function openReturnDialog(container: AdminContainerSummary): void {
    ...
    setReturnTarget(container);
    setReturnMerchantId(container.merchant?.id ?? '');
    setReturnNote('');
  }
  return <AdminShell>
