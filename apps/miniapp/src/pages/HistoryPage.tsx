      ...
      <section className="history-list">
        {transactions.map((transaction) => (
          <article className="history-row" key={transaction.id}>
            ...
            <div className="history-amount"><strong>{formatLiters(transaction.actual_liters)}</strong><span className={`quality quality-${transaction.quality.toLowerCase()}`}>{transaction.quality === 'PASS' ? 'Đạt' : 'Cần kiểm tra'}</span></div>
