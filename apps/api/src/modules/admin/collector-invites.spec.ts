...
function config(): ConfigService {
  return {
    get: jest.fn((name: string) => name === 'ZALO_OAUTH_SUCCESS_REDIRECT_URL' ? 'https://miniapp.example.test/' : undefined),
  } as unknown as ConfigService;
}

describe('collector invitation flow', () => {
  it('creates a pending collector without accepting a Zalo ID from the request', async () => {
    const input = { name: 'Người thu gom thử nghiệm', phone: '0900000000', vehicle_type: 'Xe máy', max_capacity_l: 100, ward_ids: [wardId] };
    expect(() => adminCollectorCreateSchema.parse({ ...input, zalo_id: 'must-not-be-accepted' })).toThrow();
    ...
    expect(create).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({ id: expect.any(String), userId: null, linkStatus: 'PENDING_LINK', contactPhone: input.phone }),
    }));
    expect(create.mock.calls[0][0].data).not.toHaveProperty('zaloId');
    expect(redis.setOneTime).toHaveBeenCalledWith(expect.stringMatching(/^auth:collector:invite:[a-f0-9]{64}$/), expect.any(String), expect.any(Number));
  ...
  it('links the authenticated user by internal ID and issues a collector session atomically', async () => {
    ...
    const currentUser = { id: userId, zaloId: 'zalo-from-profile', phone: null, name: 'Collector', role: Role.MERCHANT, deletedAt: null, merchant: null, collector: null };
    const linkedUser = { ...currentUser, role: Role.COLLECTOR, collector: { id: collectorId }, station: null };
    ...
    const redis = {
      ...
      restoreOneTime: jest.fn(),
    } as unknown as RedisService;
    const jwt = { signAsync: jest.fn().mockResolvedValue('jwt-fixture') };
    const auth = new AuthService(prisma, jwt as never, { getOrThrow: jest.fn().mockReturnValue('jwt-secret') } as never, {} as never, {} as never, redis);

    const session = await auth.acceptCollectorInvite({ sub: userId, role: Role.MERCHANT }, inviteCode);
    expect(tx.user.findUnique).toHaveBeenCalledWith(expect.objectContaining({ where: { id: userId } }));
    expect(updateMany).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ id: collectorId, userId: null }) }));
    expect(userUpdate).toHaveBeenCalledWith({ where: { id: userId }, data: { role: Role.COLLECTOR } });
    expect(session.user).toMatchObject({ id: userId, zalo_id: 'zalo-from-profile', role: Role.COLLECTOR, collectorId: collectorId });
  ...
  it('rejects a merchant with an existing merchant profile and preserves the invite for retry', async () => {
    ...
    const prisma = {
      ...
      $transaction: jest.fn(async (callback: (transaction: unknown) => Promise<unknown>) => callback({
        user: { findUnique: jest.fn().mockResolvedValue({ id: userId, role: Role.MERCHANT, deletedAt: null, merchant: { id: 'merchant-1' }, collector: null }) },
      })),
    } as unknown as PrismaService;
    const redis = {
      consumeOneTime: jest.fn().mockResolvedValue(collectorId),
    ...
    await expect(auth.acceptCollectorInvite({ sub: userId, role: Role.MERCHANT }, inviteCode)).rejects.toBeInstanceOf(ConflictException);
    expect(redis.restoreOneTime).toHaveBeenCalledWith(expect.stringMatching(/^auth:collector:invite:/), collectorId, expect.any(Number));
  });
  ...
  it('allows only one concurrent consumer of the same invite', async () => {
    ...
    const tx = {
      user: { findUnique: jest.fn().mockResolvedValue({ id: userId, zaloId: 'zalo-concurrent', phone: null, name: 'Collector', role: Role.MERCHANT, deletedAt: null, merchant: null, collector: null, station: null }), update: jest.fn() },
      collector: { updateMany: jest.fn().mockResolvedValue({ count: 1 }) },
      refreshToken: { create: jest.fn().mockResolvedValue({}) },
    };
    const prisma = {
      collector: { findUnique: jest.fn().mockResolvedValue(invitation) },
      user: { findUnique: jest.fn().mockResolvedValue({ id: userId, zaloId: 'zalo-concurrent', phone: null, name: 'Collector', role: Role.COLLECTOR, deletedAt: null, merchant: null, collector: { id: collectorId }, station: null }) },
      $transaction: jest.fn(async (callback: (transaction: typeof tx) => Promise<unknown>) => callback(tx)),
    } as unknown as PrismaService;
  ...
  it('rejects an expired invite without linking by phone or restoring an expired key', async () => {
    ...
    const hash = createHash('sha256').update(inviteCode).digest('hex');
    const redis = { consumeOneTime: jest.fn().mockResolvedValue(collectorId), restoreOneTime: jest.fn() } as unknown as RedisService;
    const prisma = {
      collector: { findUnique: jest.fn().mockResolvedValue(collectorRow({ inviteCodeHash: hash, inviteExpiresAt: new Date(Date.now() - 1000) })) },
    } as unknown as PrismaService;
    const auth = new AuthService(prisma, {} as never, {} as never, {} as never, {} as never, redis);

    await expect(auth.acceptCollectorInvite({ sub: userId, role: Role.MERCHANT }, inviteCode)).rejects.toMatchObject({ response: { code: 'COLLECTOR_INVITE_INVALID' } });
    expect(redis.restoreOneTime).not.toHaveBeenCalled();
  });
  ...
  it('rejects a previously consumed invite', async () => {
    ...
    const auth = new AuthService({} as PrismaService, {} as never, {} as never, {} as never, {} as never, redis);

    await expect(auth.acceptCollectorInvite({ sub: userId, role: Role.MERCHANT }, 'already-consumed-fixture')).rejects.toMatchObject({ response: { code: 'COLLECTOR_INVITE_INVALID' } });
  });
});
